...
let BASE = localStorage.getItem('unified-base') || 'https://unified-ai.hwmnbn.me/v1';
...
export function setHostAddress(host) {{
  // host can be a full base like https://unified-ai.hwmnbn.me/v1
  if (!host) return;
  BASE = host.endsWith('/v1') ? host : host.replace(/\\/$/, '') + '/v1';
  localStorage.setItem('unified-base', BASE);
}}
...
function workerUrl(path) {{
  ...
  return path.startsWith('/v1') ? (root + path) : (root + '/v1' + path);
}}

export async function getModels() {{
  const r = await fetch(workerUrl('/v1/models'), {{
...
export function postRequest(data, signal) {{
  ...
  return fetch(workerUrl('/v1/chat/completions'), {{
    method: 'POST',
    headers: {{
